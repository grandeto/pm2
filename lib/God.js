...
God.init = function() {
  ...
  //vrchats
  if (process.env.CLUSTER_MODE != 1 && process.env.CLUSTER_MODE != 0) {
      console.error('env var CLUSTER_MODE is undefined properly. Exiting...')
      process.exit()
  }
  if (process.env.CLUSTER_MODE == 1) {
      const vrchatsCluster = require('@grandeto/vrchats/cluster')
      vrchatsCluster.socketConsumerInit(God.bus)
  }
  //vrchats
}
